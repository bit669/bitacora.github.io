  ...
  if (bases.length > 0) {
    const promises = bases.map(src => {
      return new Promise((resolve, reject) => {
        ...
        img.onload = function () {
          resolve(img);
          // Se movió la adición de página nueva aquí para evitar páginas en blanco
          if (yPosImagenes + img.naturalHeight / 10 > pdf.internal.pageSize.height) {
            pdf.addPage();
            yPosImagenes = 20; // Restablece la posición Y para la nueva página
          }
          yPosImagenes += 10; // Espacio antes de la imagen
          pdf.addImage(img.src, "JPEG", 10, yPosImagenes, img.naturalWidth / 10, img.naturalHeight / 10);
          yPosImagenes += img.naturalHeight / 10 + 10; // Espacio después de la imagen
        };
